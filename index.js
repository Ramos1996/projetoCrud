...
const users = ["001", "Luciano", "lu@gmail.com"]
...
function checkuserInArray(req, res, next) {
  ...
  if (!user) {
    return res.status(400).json({ error: "esse nome não existe" });
  }
...
server.post("/users", checkUserIdExists, (req, res) => {
  const {id,name,e_mail } = req.body;
...
server.put(
//  "/users/:index", (req, res) => {
  "/users/:index", checkuserInArray, checkUserIdExists, (req, res) => {
    const { index } = req.params;
    const {id,name,e_mail } = req.body;

    users[index] =id,name,e_mail; // Substitui no vetor o nome que está na posição do 
